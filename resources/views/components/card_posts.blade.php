        ...
        <div class="card-body">
            ...
            @else
                {{-- <img class="img-fluid my-3"
                    src="{{ Storage::url($item->image->path ?? null) }}" alt=""> --}}
                @if ($item->image)
            ...
            @endif
            <x-updated :date="$item->created_at" :name="$item->user->name" :user-id="$item->user->id"
                :avatar="$item->user->image ? $item->user->image->url_image_path() : null">
            {{-- @if ($item->user->image)
                <img class="img-thumbnail avatar my-3" src="{{ $item->user->image->url_image_path() }}" alt="">
            @endif  --}}
            Added
            </x-updated>
